        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header bg-transparent border-0 text-dark">
                    ...
                    <a href="{{route('users.create')}}" class="btn btn-primary btn-user">
                        Tambah User
                    </a>
                                ...
                                @forelse ($users as $user)
                                    <tr>
                                        ...
                                        <td class="d-flex jutify-content-center">
                                            <a href="{{route('users.edit', $user->id)}}" class="btn btn-sm btn-outline-warning"><i class="fas fa-pencil-alt"></i></a>
                                            <form id="delete-form-{{ $user->id }}" action="{{ route('users.destroy', $user->id) }}" class="d-none" method="post">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            <button onclick="deleteForm('{{$user->id}}')" class="btn btn-sm btn-outline-danger mx-2"><i class="fas fa-trash"></i></button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
